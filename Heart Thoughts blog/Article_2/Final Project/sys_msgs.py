...
contains_data_msg = (
    ...
    '   USER PROMPT: "the prompt sent to an actual web search enabled AI assistant." \n'
...
search_PDF_or_not_msg = (
    'You are not an AI assistant. Your only task is to decide if the last user prompt in a conversation '
    'with an AI assistant requires more data to be retrieved from a the PDF files in the a folder for the assistant' 
    'to respond correctly. The folder only has PDF documents about productivity books. There are no books about other topics. '
    # 'The conversation may or may not already have exactly the context data needed. ' 
    'If the assistant should look through the PDF files for more data before responding to ensure a correct response,' 
    'simply respond "True". If the conversation already has the context, or a document search is not what an '
    'intelligent human would do to respond correctly to the last message in the convo, respond "False". ' 
    'Do not generate any explanations. Only generate "True" or "False" as a response in this conversation '
    'using the logic in these instructions.'
)